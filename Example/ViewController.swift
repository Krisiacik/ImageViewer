...
//        galleryItem = GalleryItem.video(fetchPreviewImageBlock: { $0(UIImage(named: "2")!) }, videoURL: URL (string: "http://video.dailymail.co.uk/video/mol/test/2016/09/21/5739239377694275356/1024x576_MP4_5739239377694275356.mp4")!)
        ...
        let itemViewControllerBlock: ItemViewControllerBlock = { index, itemCount, fetchImageBlock, configuration, isInitialController in
          let videoURL = URL(string: "http://video.dailymail.co.uk/video/mol/test/2016/09/21/5739239377694275356/1024x576_MP4_5739239377694275356.mp4")
          let vc = VideoPlayerViewController(videoURL, index: index, itemCount: itemCount, fetchImageBlock: myFetchImageBlock, configuration: configuration, isInitialController: isInitialController)
...
class CustomVideoView: UIView, ItemView {
  var image: UIImage?
...
class VideoPlayerViewController: ItemBaseController<CustomVideoView> {
  var videoURL: URL?
  let playerVC = CustomAVPlayerViewController()
  var player: AVPlayer?

  public init(_ videoURL: URL?, index: Int, itemCount: Int, fetchImageBlock: @escaping FetchImageBlock, configuration: GalleryConfiguration, isInitialController: Bool) {
  ...
  override func viewDidLoad() {
    ...
    layoutPlayer()
    playerVC.onViewDidAppear = {[weak self] in
  ...
  private func layoutPlayer() {
    playerVC.view.frame = CGRect(x: 0, y: 0, width: view.bounds.width, height: 260)
    playerVC.view.center = view.center
  }
}
