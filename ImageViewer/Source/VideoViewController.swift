...
class VideoViewController: UIViewController, ItemController, UIGestureRecognizerDelegate, UIScrollViewDelegate {
    ...
    init(index: Int, itemCount: Int, previewImage: UIImage, videoURL: NSURL, displacedViewsDatasource: GalleryDisplacedViewsDatasource?, configuration: GalleryConfiguration, isInitialController: Bool = false) {
        ...
        self.videoView.hidden = isInitialController
    ...
    override func viewDidLoad() {
        ...
        self.videoView.image = previewImage
    }
    ...
    override func viewDidLayoutSubviews() {
        ...
        videoView.bounds.size = aspectFitSize(forContentOfSize: videoView.image!.size, inBounds: self.scrollView.bounds.size)
        scrollView.contentSize = videoView.bounds.size

        videoView.center = scrollView.boundsCenter
    }
    ...
    func scrollViewDidZoom(scrollView: UIScrollView) {
        ...
        videoView.center = contentCenter(forBoundingSize: scrollView.bounds.size, contentSize: scrollView.contentSize)
    }
    ...
    func scrollViewDidDoubleTap(recognizer: UITapGestureRecognizer) {
        ...
        let touchPoint = recognizer.locationOfTouch(0, inView: videoView)
        let aspectFillScale = aspectFillZoomScale(forBoundingSize: scrollView.bounds.size, contentSize: videoView.bounds.size)
    ...
    func scrollViewDidSwipeToDismiss(recognizer: UIPanGestureRecognizer) {
        ...
        guard videoView.image != nil else {  return }
    ...
    func handleSwipeToDismissEnded(swipeOrientation: SwipeToDismiss, finalVelocity velocity: CGPoint, finalTouchPoint touchPoint: CGPoint) {
        ...
        case (.Vertical, _) where velocity.y < -thresholdVelocity:
            ...
            swipeToDismissTransition?.finishInteractiveTransition(swipeOrientation,
                                                                  ...
                                                                  targetOffset: (view.bounds.height / 2) + (videoView.bounds.height / 2),
        ...
        case (.Horizontal, 0) where thresholdVelocity < velocity.x:
            ...
            swipeToDismissTransition?.finishInteractiveTransition(swipeOrientation,
                                                                  ...
                                                                  targetOffset: -(view.bounds.width / 2) - (videoView.bounds.width / 2),
        ...
        ///If none of the above cases apply, we cancel.
        default:
    ...
    func animateDisplacedImageToOriginalPosition(duration: NSTimeInterval, completion: ((Bool) -> Void)?) {
        ...
        UIView.animateWithDuration(duration, animations: {  [weak self] in
            ...
            if isPortraitOnly() {
                self?.videoView.transform = CGAffineTransformInvert(rotationTransform())
            }
        ...
        if presentationStyle == .Displacement {
            ...
            if let displacedView = self.findVisibleDisplacedView() {
                ...
                UIView.animateWithDuration(reverseDisplacementDuration, animations: {  [weak self] in
                    ...
                    self?.videoView.frame = displacedView.frame(inCoordinatesOfView: self!.view)
                    self?.videoView.clipsToBounds = true
                    self?.videoView.contentMode = displacedView.contentMode
                    }, completion: { _ in
    ...
    override func observeValueForKeyPath(keyPath: String?, ofObject object: AnyObject?, change: [String: AnyObject]?, context: UnsafeMutablePointer<Void>) {
        ...
        case .Horizontal:
            ...
            distanceToEdge = (scrollView.bounds.width / 2) + (videoView.bounds.width / 2)
            percentDistance = fabs(scrollView.contentOffset.x / distanceToEdge)
            
        case .Vertical:
            
            distanceToEdge = (scrollView.bounds.height / 2) + (videoView.bounds.height / 2)
