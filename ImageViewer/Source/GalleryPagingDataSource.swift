...
final class GalleryPagingDataSource: NSObject, UIPageViewControllerDataSource {
    ...
    func createItemController(_ itemIndex: Int, isInitial: Bool = false) -> UIViewController {
        ...
        case .video(let fetchImageBlock, let videoURL, let videoProgress):

            let videoController = VideoViewController(index: itemIndex, itemCount: itemsDataSource.itemCount(), fetchImageBlock: fetchImageBlock, videoURL: videoURL, videoProgress: videoProgress, scrubber: scrubber, configuration: configuration, isInitialController: isInitial)
