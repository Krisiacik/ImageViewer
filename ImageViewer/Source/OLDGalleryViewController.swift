//
//  OLDGalleryViewController.swift
//  ImageViewer
//
...
final public class OLDGalleryViewController : UIPageViewController, UIViewControllerTransitioningDelegate, OLDImageViewControllerDelegate  {
    ...
    private var pagingDatasource: OLDGalleryPagingDatasource!
    ...
    /// IMAGE VC FACTORY
    private var imageControllerFactory: OLDImageViewControllerFactory!
    ...
    public init(startIndex: Int, itemsDatasource: GalleryItemsDatasource, displacedViewsDatasource: GalleryDisplacedViewsDatasource? = nil, configuration: GalleryConfiguration = []) {
        ...
        self.imageControllerFactory = OLDImageViewControllerFactory(itemsDatasource: itemsDatasource, displacedViewsDatasource: displacedViewsDatasource, startIndex: startIndex, configuration: configuration, fadeInHandler: fadeInHandler, delegate: self)
        
        /// Needs to be kept alive with strong reference
        self.pagingDatasource = OLDGalleryPagingDatasource(imageControllerFactory: imageControllerFactory,
    ...
    func configureInitialImageController(configuration: GalleryConfiguration) {
        ...
        let initialImageController = OLDImageViewController(itemsDatasource: itemDatasource, displacedViewsDatasource: displacedViewsDatasource, configuration: configuration, startIndex: startIndex, imageIndex: startIndex, fadeInHandler: fadeInHandler, delegate: self)
        ...
        UIView.animateWithDuration(decorationViewsDismissAnimationDuration, animations: { [weak self] in
            ...
            self?.closeButton?.alpha = 0.0
                ...
                case true: /// We use the reverse-displacement animation ie. image returns back to its original position in parent view. (Not to be confused with swipe-to-dismiss animation.)
                    ...
                    if let imageController = self?.viewControllers?.first as? OLDImageViewController {
